...
const createDateHierarchy = (field) => {
  ...
  const createNumberField = (mode) => {
    const name = mode === "Count" ? "Count_Rows" : `${selectedCol}_${mode}`;

    const fieldName = addDerivedField(name, (row) => {
      if (mode === "Count") return 1;
      return Number(row[selectedCol]) || 0;
    });
    ...
    useField(fieldName, (f) => ({
      ...
      aggregation,
      sortBy: f,
      sort: mode === "SortDesc" || mode === "Top10" ? "desc" : "none",
      limit: mode === "Top10" ? 10 : null,
    }));
  };
  ...
  return (
    <div className="w-full h-full flex flex-col">
      ...
      {contextMenu.visible && selectedCol && (
        <div
          className="fixed bg-white border shadow-md rounded-md z-50 p-1 w-64"
          style={{ top: contextMenu.y, left: contextMenu.x }}
          onMouseLeave={closeMenu}
        >
          <div className="px-2 py-1 text-xs font-bold text-slate-400 border-b mb-1">
            {selectedCol}
          </div>
          ...
          {selectedType === "number" && (
            <>
              <MenuButton onClick={() => useField(selectedCol, (f) => ({ y: [f] }))}>
                Add to Y Axis
              </MenuButton>
              <MenuButton onClick={() => createNumberField("Sum")}>Sum {selectedCol}</MenuButton>
              <MenuButton onClick={() => createNumberField("Average")}>Average {selectedCol}</MenuButton>
              <MenuButton onClick={() => createNumberField("Count")}>Count Rows</MenuButton>
              <MenuButton onClick={() => createNumberField("Top10")}>Show Top 10</MenuButton>
              <MenuButton onClick={() => createNumberField("SortDesc")}>Sort Descending</MenuButton>
            </>
          )}
          ...
          {selectedType !== "number" && selectedType !== "date" && (
            <>
              ...
              <MenuButton onClick={() => createTextField("Top10")}>Show Top 10 {selectedCol}</MenuButton>
              <MenuButton onClick={() => createTextField("Top20")}>Show Top 20 {selectedCol}</MenuButton>
...
function MenuButton({ children, onClick }) {
  return (
    <button
      className="block w-full text-left px-2 py-1.5 text-sm hover:bg-slate-100 rounded"
      onClick={onClick}
    >
      {children}
    </button>
